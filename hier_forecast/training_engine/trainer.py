...
from hier_forecast.training_engine.utils import seed_worker, setup_wandb_auth


def train_and_validate(
    seeds: tuple[int, ...] = (42, 43, 44),
    lr: float = 0.0005066474601108174,
    hidden_size: int = 128,
    num_blocks: int = 8,
    dropout: float = 0.1,
    batch_size: int = 1024,
    num_batches_per_epoch: int = 200,
    wandb_project: str = "tsmixer-m5",
        ...
        with wandb.init(
            ...
            for epoch in epoch_bar:
                ...
                if epoch % EVAL_FREQ == 0 or epoch == 1:
                    val_wrmsse = evaluate_wrmsse(data_dict, model, device=str(device), split="val")
            ...
            seed_test_wrmsse = evaluate_wrmsse(data_dict, model, device=str(device), split="test")
    ...
    mean_wrmsse = float(np.mean(test_wrmsse_scores))
    std_wrmsse = float(np.std(test_wrmsse_scores))
