...
class ModelRunner:
    ...
    def from_wandb(
        ...
        if not weights_path or not os.path.exists(weights_path):
            ...
            elif local_dir in ("./artifact", "./artifacts", ".") and os.path.exists("best_wrmsse_seed_42.pth"):
        ...
        elif os.path.exists("category_maps.json"):
            with open("category_maps.json", encoding="utf-8") as f:
                category_maps = json.load(f)
        else:
            category_maps = {}
    ...
    @staticmethod
    def quantiles(mu: np.ndarray, alpha: np.ndarray) -> dict[str, list]:
        """Compute exact discrete p10, p50 (median), and p90 quantiles via NegativeBinomial.ppf."""
        ...
        p10_list = p10.tolist() if isinstance(p10, np.ndarray) else [int(p10)]
        p50_list = p50.tolist() if isinstance(p50, np.ndarray) else [int(p50)]
        p90_list = p90.tolist() if isinstance(p90, np.ndarray) else [int(p90)]
