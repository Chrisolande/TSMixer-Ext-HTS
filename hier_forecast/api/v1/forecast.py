...
from hier_forecast.api.dependencies import get_inference_store, get_model_runner
from hier_forecast.api.runner import ModelRunner
from hier_forecast.api.schemas.request import ForecastRequest
from hier_forecast.api.schemas.response import (
    ErrorDetail,
    ForecastResponse,
    ItemForecastResult,
    Quantiles,
...
def run_batch_forecast(
    ...
    for idx, item in enumerate(request_data.items):
        try:
            tensors = store.build_tensors(item, as_of_date=request_data.as_of_date)
    ...
    if valid_tensors:
        ...
        for batch_idx, original_idx in enumerate(valid_indices):
            ...
            median_val = q_dict["p50"] if quantiles_obj else None
